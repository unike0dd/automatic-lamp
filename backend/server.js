...
app.post('/create-checkout', async (req, res) => {
  try {
    ...
    const validEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(customerEmail || ''));

    if (!Array.isArray(cartItems) || cartItems.length === 0 || !isValidBusinessId(businessId) || !validEmail) {
      return res.status(400).json({
        error:
          'cartItems (non-empty array), businessId (3-64 chars: letters, numbers, hyphen), and valid customerEmail are required.',
      });
    }
    ...
    if (normalizedItems.some((item) => !item.name || !Number.isFinite(item.price) || item.price <= 0 || !Number.isInteger(item.quantity) || item.quantity <= 0)) {
      return res.status(400).json({ error: 'Each cart item must have name, price > 0, and integer quantity > 0.' });
    }
